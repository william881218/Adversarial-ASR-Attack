...
def compute_PSD_matrix(audio, window_size):
    ...
    Returns:
        PSD: np.array([shape=(1 + n_fft/2, n_frames)])
        psd_max: np.array([n_frames]), largest value of psd in each timestamp
    """

    win = np.sqrt(8.0/3.) * librosa.core.stft(audio, center=False)
    z = abs(win / window_size)
    psd_max = np.max(z*z, axis=0)
    psd = 10 * np.log10(z * z + 0.0000000000000000001)
    PSD = 96 - np.max(psd, axis=0) + psd
    return PSD, psd_max
...
def compute_threshold(PSD, barks, ATH, freqs):
    """ returns the global masking threshold of current timestamp
    """
...
def generate_th(audio, fs, window_size=2048):
    ...
    for i in range(PSD.shape[1]):
        theta_xs.append(compute_threshold(PSD[:,i], barks, ATH, freqs))
    theta_xs = np.array(theta_xs)
    #print('finish generating threshold: theta {} / psd_max {}'.format(theta_xs.shape, psd_max.shape))
